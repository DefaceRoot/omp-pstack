...
Applies to any typed language. `skill://typescript-best-practices` grounds it in TypeScript syntax.
...
- **External data is untyped until parsed.** RPC payloads, JSON, IPC messages, CLI args, config files, environment variables, database rows. Have a parse function at every boundary that turns unstructured input into the typed model. See `skill://principle-boundary-discipline` for where to put validation.
...
- **Derive types from authoritative schemas.** When a protocol buffer, OpenAPI spec, GraphQL schema, database migration, or design-system token file defines a shape, derive from it instead of hand-rolling a parallel type. See `skill://principle-encode-lessons-in-structure`.
