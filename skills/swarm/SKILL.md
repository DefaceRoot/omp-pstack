...
4. Only for a model race or mixed per-arm shape that needs explicit model selection, pick the `swarm workers` selector from the active always-applied pstack model rule already present in the OMP system prompt when that role is present. Otherwise use `auto`. Name each arm's validated selector up front.
