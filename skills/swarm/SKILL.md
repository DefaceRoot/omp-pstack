...
4. Pick the `swarm workers` selector from the active always-applied pstack model rule already present in the OMP system prompt when that role is present. Otherwise use `auto`. For a model race, name each arm's validated selector up front.
