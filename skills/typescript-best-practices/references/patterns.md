...
Code examples for each rule in `SKILL.md`. The underlying principles are language-agnostic; see `skill://principle-type-system-discipline` and `skill://principle-boundary-discipline`.
...
Validate once where data crosses in; trust types inside. See `skill://principle-boundary-discipline`.
