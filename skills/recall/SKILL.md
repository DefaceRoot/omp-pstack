...
OMP transcripts live recursively under `~/.omp/agent/sessions/` (or `$XDG_DATA_HOME/omp/sessions/`) as `<timestamp>_<uuid>.jsonl`. The first JSONL entry is a session header with `id`, `timestamp`, and `cwd`; later entries form the conversation tree. Treat the header's exact `cwd`, not a directory-name guess, as the workspace boundary.
