...
Prefer an explicit current OMP session path, transcript path, `history://` reference, or `agent://` handoff before filesystem discovery. If none resolves, run `omp config path`, trim its non-empty output as `agent_dir`, and search `<agent_dir>/sessions` recursively for `<timestamp>_<uuid>.jsonl`. For a named profile, use `OMP_PROFILE`, or `PI_PROFILE` only when `OMP_PROFILE` is unset; after validating it against `[A-Za-z0-9][A-Za-z0-9_.-]*`, also consider `$XDG_DATA_HOME/omp/profiles/<profile>/sessions` (using the platform XDG data default when the variable is unset). Never read default-profile sessions while a named profile is active. The first JSONL entry is a session header with `id`, `timestamp`, and `cwd`; later entries form the conversation tree. Treat the header's exact `cwd`, not a directory-name guess, as the workspace boundary.
