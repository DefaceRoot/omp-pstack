...
3. Fan out across chat history. First filter candidates by exact header `cwd` and modification time. Then call `pstack_task` with `strategy: "slice"`, one complete task per non-overlapping time window, and the configured fast selector or `auto`. Each slice uses `grep` for the topic, `read`s only matching regions, and skips the current session plus obvious child, eval, and test noise. Each returns one block per chat with topic, user goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), citing the session UUID. For one or two chats, search directly. Raw transcripts remain in workers; the main thread receives only findings.
