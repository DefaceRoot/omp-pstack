...
Call `pstack_task` once with `strategy: "panel"`, the shared candidate brief in `prompt`, and the N configured selectors in `models`. The extension launches one parallel OMP `poteto-agent` per panel entry. The shared prompt names the grounding path; each panel member derives its own `panel-N` output path and produces both the artifact and a short rationale.
...
The synthesized artifact has to hold up under the same scrutiny as any other output, per `skill://principle-prove-it-works`. The arena does not earn you a pass.
