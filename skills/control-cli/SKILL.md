...
- Use `read` to inspect the repo's documented commands and existing harnesses before launching anything.
- Use `bash` for short, non-interactive commands. Run an interactive CLI or any long-lived process through `hub` with PTY enabled.
- Start the process with `hub start`, supplying the executable and arguments separately. Set a readiness log pattern or port when the program exposes one.
- Capture evidence with `hub logs`, drive one input at a time with `hub send` text or terminal keys, and stop the process with `hub stop` after saving the transcript.
- Use tmux or a temporary PTY probe when the native process controls cannot exercise a required terminal behavior such as resize.

## Harness Options

- Repo-native harness: prefer checked-in scripts because they know the app's startup, env, and prompts.
- OMP `hub`: managed PTY processes, readiness checks, captured logs, serialized input, terminal keys, and process-tree cleanup.
...
## Minimal Native Harness

Start the command with OMP `hub start` in PTY mode. Wait for a concrete readiness pattern, inspect it with `hub logs`, send input through `hub send`, then capture the resulting logs before calling `hub stop`. Keep the executable and arguments separate, and use the stable process name for every operation.
