...
Companion to `skill://how`. `how` answers what the code does and how it works. `why` answers what forces led to its shape.

## Operating Posture

Operate as a careful, cautious, precise investigator. Separate cited evidence from inference, surface contradictions, and name gaps in the record. Read `skill://why/references/epistemics.md` for the confidence framework and phrasing guide. The synthesizer must follow it.
...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, what was just discussed). State your interpretation briefly so the user can redirect, then proceed.
...
Before spawning investigators, inspect the OMP session's available tool map and `mcp://` resources when present. OMP exposes configured MCP capabilities as tools or resources. Use only what that map shows.
