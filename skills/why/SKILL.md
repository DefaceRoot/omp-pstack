...
Before spawning investigators, inspect the OMP session's available tool map and `mcp://` resources when present. OMP exposes configured MCP capabilities as tools or resources; do not infer availability from a Cursor `mcps/` directory.
