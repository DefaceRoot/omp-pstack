...
Start by finding the relevant code. Use lowercase `glob` to find directories and files, `grep` to find key symbols, and `read` to understand the actual implementation. Don't guess from names. Read the code.
