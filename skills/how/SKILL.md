...
- Use `read` on the actual code; don't guess from file names
...
After the explanation is complete, call `pstack_task` once with `strategy: "panel"`, the complete critic brief in `prompt`, and the configured how-critics list in `models` (default four `auto` entries). The extension launches one parallel OMP critic per entry. The prompt uses `skill://how/references/critic-prompt.md`, forbids writes, and includes:
