...
1. Call the lowercase OMP `task` tool with one item whose `agent` is `comment-sicko`; pass the scope in the task and shared context. Do not restate the agent's rules.
...
4. Implement the smallest root-cause fix in scope. Remove every named workaround. If the root cause is out of scope, land the smallest in-scope fix and report the rest open. `skill://principle-fix-root-causes` and `skill://principle-redesign-from-first-principles` guide intent only. Neither authorizes widening the fence nor fixing instances outside it. Never bolt on symptom guards.
