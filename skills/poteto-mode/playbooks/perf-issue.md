...
3. Plan the fix from the trace. If it crosses a function boundary, run `skill://architect` first. Delegate implementation through native `task` with `agent: "pstack-perf"`. Review the diff and capture a post-fix trace.
