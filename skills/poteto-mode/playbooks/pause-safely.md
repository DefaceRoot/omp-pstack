...
**You own a clean stop. Leave a checkpoint a cold-start agent can resume from.** Pause only on an explicit request to stop or when context is about to compact. On "keep going", "going to bed, keep going", or "don't stop", continue. Autonomous run already checkpoints per iteration.
