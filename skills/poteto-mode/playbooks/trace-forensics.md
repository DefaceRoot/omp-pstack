...
6. Hand back a cited diagnosis, no fix unless asked. Route to the Bug fix (`skill://poteto-mode/playbooks/bug-fix.md`) or Perf issue (`skill://poteto-mode/playbooks/perf-issue.md`) playbook once the cause is known. Throughput checkpoint stays one line: `throughput checkpoint: n/a, read-only forensics`.
