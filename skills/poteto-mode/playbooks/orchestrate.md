...
- **Coordinator (this chat).** Frames, authors briefs, drains the inbox, owns the human report, and makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are tasks. Mechanically landing a verified unit is bookkeeping it may do when local git is cheap. Use lowercase OMP `task` with `agent: "poteto-agent"` plus `hub` for durable background sub-coordinators. Use `pstack_task` `slice` calls for model-aware worker waves and `panel` calls only for identical-brief races or independent verdicts. State reads and writes go through `skill://poteto-mode/scripts/orch/orch.ts` at drain points; the CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** One durable OMP `poteto-agent` per track, and only when the program exceeds what one coordinator can drain. It owns its track's units and boards, authors workers' briefs, spawns workers and verifiers when OMP's configured recursion depth permits it, and rolls up aggregates at wave boundaries without forwarding raw reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; use OMP background `task` jobs plus `hub` for that durable window rather than a blocking `pstack_task` batch.
- **Worker / verifier.** An OMP agent with an exclusive worktree or branch and a complete brief. Use `pstack_task` slices for finite parallel units and standard background `task` only when rolling refill or follow-up through `hub` is required. All workers can access local project state, so point at files instead of inlining bulk context. Prefer fewer, broader workers and one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different validated model selector from its worker when available.
