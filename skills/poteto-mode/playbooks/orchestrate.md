...
- **Coordinator (this chat).** Frames, authors briefs, drains the inbox, owns the human report, and makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are tasks. Mechanically landing a verified unit is bookkeeping it may do when local git is cheap. For ordinary worker waves, call native `task` once with shared `context` and stable item names. Set `agent: "poteto-agent"` on code workers and `agent: "poteto-judgment"` on judgment verifiers. Queueing finished work behind an idle stacker can leave a deadline with nothing landed. Idle native children park automatically. Use `hub` to inspect and wake or revive durable background sub-coordinators. State reads and writes go through `skill://poteto-mode/scripts/orch/orch.ts` at drain points. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** One durable OMP `poteto-agent` per track, and only when the program exceeds what one coordinator can drain. A track the coordinator can drain needs no middle layer. It owns its track's units and boards, authors workers' briefs, spawns workers and verifiers when OMP's configured recursion depth permits it, and rolls up aggregates at wave boundaries without forwarding raw reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window. Never use blocking batches, which pay for the slowest child of every batch. Use OMP background `task` jobs plus `hub` for that durable window.
- **Worker / verifier.** An OMP agent with an exclusive worktree or branch and a complete brief. For finite parallel units, call native `task` once with a shared `context` and a stable `name` for every item. Use `agent: "poteto-agent"` for code workers and `agent: "poteto-judgment"` for judgment verifiers. Include a complete reason-bearing `task`. Do not add a `model` field. All workers can access local project state, so point at files instead of inlining bulk context. Prefer fewer, broader workers and one writer per worktree or branch (principle-separate-before-serializing-shared-state). When a verifier must use a different model family, run `pstack_task` with `strategy: "slice"`, one slice, and `model: "cross-family"`.
...
Use `pstack_task` only for true model panels, explicit per-arm model selection, model races, or cross-family judges.

#### Store layout

Before the first bookkeeping command, bind the CLI to a concrete store inside the current project:

```sh
export ORCH_STORE="$PWD/.omp/orchestrate"
```

Keep that export in the coordinator environment for the full run. If the coordinator is not rooted at the project, replace `$PWD` with the project's absolute path. Every file in the store has exactly one writer. Owners publish facts, readers aggregate at read time. Use `bun skill://poteto-mode/scripts/orch/orch.ts` for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape. A 4KB scaffold around a two-line edit costs more to write and obey than the edit. OMP workers may reference the standing-orders file by store path. Paste it verbatim into every fresh spawn and every resumed durable agent.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget, OMP agent choice, panel/slice strategy, and isolation list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm that watcher with OMP `/goal set`, with a long heartbeat fallback), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack. Record the holder in the standing orders. Run restacks only in the stacker's isolated OMP worktree. Serialize them and bound resource concurrency so a large restack cannot starve the coordinator.
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts. A dedicated verifier agent is for units whose verification is expensive, judgment-laden, or high-blast-radius. When a different family is required, use the cross-family judge described in Roles. A verifier agent whose entire product would be rerunning one command is ceremony, not verification.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the agent's status in OMP Agent Hub (`hub` roster/jobs). Transcript mtime is not liveness.
...
- Retry by mode: cap-hit or oom, respawn with smaller scope. Network-drop, retry as-is. Tool-error, retry through a different assigned role. Unknown, retry once. Two retries, then abandon the unit and replan around it.
...
- After an OMP restart, treat in-process agents and jobs as dead. Re-read the standing orders and `units.tsv`, recompute the frontier, recover completed work from pushed branches and PRs rather than agent ids, respawn one sub-coordinator per active track from its stored brief plus current state, drain, and resume. The dead session's store lock clears itself on the next write. `orch` replaces a lock whose holder pid is gone.
