...
- **Worker / verifier.** An OMP agent with an exclusive worktree or branch and a complete brief. For finite parallel units, call native `task` once with a shared `context` and a stable `name` for every item. Use `agent: "poteto-agent"` for code workers and `agent: "pstack-cross-judge"` for judgment verifiers and independent second opinions. Include a complete reason-bearing `task`. Do not add a `model` field. All workers can access local project state, so point at files instead of inlining bulk context. Prefer fewer, broader workers and one writer per worktree or branch (principle-separate-before-serializing-shared-state). The cross-judge selects a different model family from the spawning session when possible.
...
A sub-coordinator brief adds its track boundary and unit list, its spawn budget, roster agent choice, and isolation list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts. A dedicated verifier agent is for units whose verification is expensive, judgment-laden, or high-blast-radius. When a different family is required, use native `task` with `agent: "pstack-cross-judge"`. A verifier agent whose entire product would be rerunning one command is ceremony, not verification.
