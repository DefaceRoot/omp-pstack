...
3. Migrate one component at a time. Parallelize across worktrees, one owner per component (`skill://principle-separate-before-serializing-shared-state`). Shared primitives migrate first as a blocking phase.
4. Verify each component against its baseline via image diff on the matching surface via the control skill. A nonzero diff is a fail. Investigate the pixel delta. Use OMP's `/goal set` per component until the diff is zero.
5. Run `skill://poteto-mode/playbooks/opening-a-pr.md` per component or per safe batch.
