...
4. **Spawn N parallel candidates** per `skill://arena`'s Phase B. For three arms, use a `pstack_task` panel with `strategy: "panel"` and `models: ["@pstack-judgment", "@pstack-precise", "@pstack-code"]`. For another arm count, repeat role aliases or use explicit selectors the user chose. Each candidate works in its own sanitized dir with the same prompt.
5. **Spawn one blinded judge** on a different model family per `skill://arena`'s Phase C. Use `pstack_task` with `strategy: "slice"`, one slice, and `model: "cross-family"`. The judge sees outputs by sanitized label and the rubric, never a model name.
