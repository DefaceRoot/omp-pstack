...
6. **Verify the chain from transcripts, not self-report.** Use each OMP assignment's `history://` reference when available. Otherwise locate its JSONL recursively under `~/.omp/agent/sessions/` (or `$XDG_DATA_HOME/omp/sessions/`) by exact header `cwd` and child id. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. If a one-shot `pstack_task` result has no persisted transcript, mark chain verification unavailable rather than inventing it.
