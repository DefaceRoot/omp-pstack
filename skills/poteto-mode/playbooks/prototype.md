...
1. Scope the decision the prototype exists to make: which layout, which interaction, which density, or for an empirical fork which behavior, timing, or approach. No decision means no prototype. Route to the Feature playbook (`skill://poteto-mode/playbooks/feature.md`).
...
6. Present alternatives, tradeoffs, and a recommendation. The output is the decision plus the throwaway artifact, not shippable code. Hand the chosen direction to the Feature playbook (`skill://poteto-mode/playbooks/feature.md`) or `skill://architect` for the shape.
