...
2. Settle open questions by prototype before you write. Follow `skill://poteto-mode/playbooks/prototype.md` for each. Keep the branch, the SHA, and the screenshots for Appendix A. Ask the operator only about a product or preference call that no run can settle. Give options (`skill://principle-never-block-on-the-human`).
3. Explore in native `task` subagents with `agent: "poteto-agent"` per the Subagents section (`skill://principle-guard-the-context-window`). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
4. Use OMP `/plan` to draft or revise the plan, then copy the skeleton below into the plan file and fill every placeholder. Unless the operator names a path, write the file under the project's `.omp/plans/`. Keep every heading and every sub-block in the order shown. One section per PR. One PR is one change with its own evidence (`skill://principle-sequence-verifiable-units`). Name the execution playbook in **How to read this**. Pick between `skill://poteto-mode/playbooks/autopilot-full.md` and `skill://poteto-mode/playbooks/autopilot-stack.md` per the rule at the end of `skill://poteto-mode/playbooks/autopilot-stack.md`. A standing program takes `skill://poteto-mode/playbooks/orchestrate.md`.
...
The program runs <execution playbook URI, such as `skill://poteto-mode/playbooks/autopilot-full.md`>. <Who merges, and which PR ids are the operator's items that stop at merge-ready.>
...
- [ ] On the operator's go, arm OMP `/goal set` with this exact text. "<The plan path, the PR ids in order, the verification rule, who merges, and the done condition.>"
- [ ] Read these installed skills at program start. Re-read them at every tick.
  - [ ] `skill://poteto-mode/playbooks/autopilot-full.md` or `skill://poteto-mode/playbooks/autopilot-stack.md`, or `skill://poteto-mode/playbooks/orchestrate.md` for a standing program
  - [ ] `skill://swarm`
  - [ ] the matching control skill
  - [ ] `skill://poteto-mode/playbooks/opening-a-pr.md`
  - [ ] <each other installed leaf skill the program uses>
- [ ] Arm a 30-minute audit tick with OMP `/goal set` and a fallback heartbeat. Never leave the cadence to memory.
- [ ] Use this tick prompt, verbatim. "Re-read the installed execution playbook and the armed /goal set. Audit the operation against both and fix drift in this tick. Probe every active lane and judge progress by side effects only. Stand down a stuck lane and dispatch its replacement now. Then post a short status message to the operator in the session only when the audit found a tracked change that no earlier status message reported, such as a PR opened, a code-ready head, a round launched or closed, a verdict, a merge, a stuck agent and the action taken, a blocker added or cleared, or a decision only the operator can make. Name every such change and nothing else. Do not repeat a table, the merged list, or an unchanged blocker. If the audit found none, end the turn with no reply text. Either way, log this tick's row in your decision trail. The row names the items reported, or none."
- [ ] On the operator's hold or stand-down, send every owner a zero-writes order at once.

### Spawn owners

- [ ] Spawn one native `task` owner per PR with `agent: "poteto-agent"`, a stable name, shared context, an exclusive worktree, and the full lifecycle the execution playbook names.
...
Each live lane uses native `task` with `agent: "pstack-swarm-worker"` and an isolated worktree at the PR head. Drive through `skill://control-ui` or `skill://control-cli` from the bundled team-kit subset.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes with the `pstack-swarm-worker` agent at the PR head, per the boot recipe.
