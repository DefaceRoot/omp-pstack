...
Each live lane uses native `task` with `agent: "poteto-agent"` and an isolated worktree at the PR head. Drive through `skill://control-ui` or `skill://control-cli` from the bundled team-kit subset.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten lanes with the `poteto-agent` agent at the PR head, per the boot recipe.
