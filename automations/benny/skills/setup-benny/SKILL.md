...
The human enters setup by asking an OMP agent to `read` the pack's `FOR_AGENTS.md`. The bootstrap copies the whole pack into the target repository, then reads this file directly at `.omp/automations/benny/skills/setup-benny/SKILL.md`.
...
Ask which repository will run the automations. The source pack is the directory containing `FOR_AGENTS.md`. The destination is `<target-repository>/.omp/automations/benny/`.
...
From the target repository, run `omp plugin enable @defaceroot/omp-pstack --scope project`. Verify the exact enabled package with `omp plugin list --json`. OMP owns `.omp/plugins/installed_plugins.json`; never synthesize or hand-edit its schema. Preserve every unrelated installed plugin.
...
Tell the user that `.omp/plugins/installed_plugins.json`, `.omp/automations/benny/`, and any referenced secret-free configuration must be committed before either automation is enabled. Do not commit them unless the user asks.
...
Create user-owned copies outside `.omp/automations/benny/`. These are configuration files, not pack files. Example locations:

- Project config, such as `.omp/benny/configuration.yaml`
- Project feature map, such as `.omp/benny/feature-map.md`
- Project routing map, such as `.omp/benny/routing.md`
...
Run `omp models --json` and use only exact `models[].selector` values it returns, plus the `auto` or `inherit-parent` aliases. Do not guess a selector or carry over a private default.
...
1. Copy `../triage-issue-reports/references/routing.example.md` outside `.omp/automations/benny/`.
...
3. Tell the live prompt to read and follow its exact committed operational file under `.omp/automations/benny/`.
...
- Name `benny-triage`.
- Read and follow `.omp/automations/benny/skills/triage-issue-reports/SKILL.md` for every run.
...
- Read and follow `.omp/automations/benny/skills/reproduce-and-fix-issues/SKILL.md` for every run.
...
- Direct instruction to read `.omp/automations/benny/skills/triage-issue-reports/SKILL.md`
...
- Name and description
- Direct instruction to read `.omp/automations/benny/skills/reproduce-and-fix-issues/SKILL.md`
